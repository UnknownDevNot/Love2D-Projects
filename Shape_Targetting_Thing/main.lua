...
function love.load()
    ...
    PARTICLE_SIZE = 6
    SHAPE_SIZE = 32
    GOOD_IMAGE = love.graphics.newImage("images/goodSquare.png")
    BAD_IMAGE = love.graphics.newImage("images/badSquare.png")
    ...
    love.window.setIcon(love.image.newImageData("images/IconSTT.png"))
    print("Window Initialized")
end

function love.update(dt)
    if (initialBootup) then
        return
    end
    local badSquares = 0
...
function love.draw()
    if (initialBootup) then
        love.graphics.printf("Shape Targetting... Thing", 0, 160, WINDOW_WIDTH, "center")
        love.graphics.printf("Press enter to play... You know.", 0, 180, WINDOW_WIDTH, "center")
        return
    end
    -- Draw the shapes
    for index, shape in ipairs(shapeSprites) do
        ...
        if (shape.innocent) then
            ...
            imageToDraw = GOOD_IMAGE
        else
            love.graphics.setColor(brightness, 0, 0, 1)
            imageToDraw = BAD_IMAGE
        end
        ...
        love.graphics.draw(imageToDraw, shape.x, shape.y)
    end
...
function love.keypressed(key)
    if (key == "return" and gameState == "GameOver" or initialBootup) then
        shapeSprites = {}
        particleSprites = {}
        currentRound = 0
        startNewRound()
