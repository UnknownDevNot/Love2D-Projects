...
function initGlobals()
    playerSprite = {x = 225, y = 165, width = 30, height = 30, dead = false}
    circleSprites = {}
    particleSprites = {}
    playerScore = 0
    spawnTimer = 0.1
...
function love.load()
    ...
    CIRCLE_RADIUS = 10
    PARTICLE_SIZE = 5
    PLAYER_IMAGE = love.graphics.newImage("images/playerImg.png")
    CIRCLE_IMAGE = love.graphics.newImage("images/circleImg.png")
    print("Constants initalized")
    -- Init globals for first time.
    initGlobals()
    ...
    love.window.setIcon(love.image.newImageData("images/iconCDE.png"))
...
function love.draw()
    ...
    if (not playerSprite.dead) then
        ...
        love.graphics.draw(PLAYER_IMAGE, playerSprite.x, playerSprite.y)
    end
    -- Draw circles.
    love.graphics.setColor(1, 0, 0, 1)
    for index, circle in ipairs(circleSprites) do
        love.graphics.ellipse("fill", circle.x, circle.y, CIRCLE_RADIUS, CIRCLE_RADIUS)
        love.graphics.draw(CIRCLE_IMAGE, circle.x - CIRCLE_RADIUS, circle.y - CIRCLE_RADIUS)
    end
    ...
    if (playerSprite.dead) then
        if (not initialBootup) then
            ...
            love.graphics.printf("CIRCLE DODGING EXPERIENCE", 0, 160, 480, "center")
            love.graphics.printf("Press enter to play the game.", 0, 180, 480, "center")
        end
    end
end
